price=read.csv('/Users/deniz.kilic/Desktop/Big Data/R Denemeler/ASELSAN Stocks/asels_table.csv',header=TRUE)
# price=price[order(price$Date,decreasing=FALSE),]
trend=read.csv("/Users/deniz.kilic/Desktop/Big Data/R Denemeler/ASELSAN Stocks/asels_multiTimeline.csv",header=TRUE)
plot(trend$ASELSAN,type='l')
plot(price$Close,type='l',xlab='Time',ylab='ClosePrices',main='Weekly Close Prices of aapl')

gootrend = trend$ASELSAN
...
correlation = cor(ret[1:261],trendret[4:264])
correlation
correlation = cor(a[1:261],trend$ASELSAN[5:265])
correlation
correlation = cor(a[1:261],trend$ASELSAN[1:261])
correlation
correlation = cor(a[1:261],trend$ASELSAN[1:261])
correlation

d_1=data.frame(ret[1:261],trendret[4:264])
mod1=lm(ret[1:261]~trendret[4:264], data=d_1)
summary(mod1)

d_2=data.frame(d1[1:261],trend$ASELSAN[5:265])
mod2=lm(d1[1:260]~trend$ASELSAN[6:265], data=d_2)
summary(mod2)
plot(trend$ASELSAN[5:265],d1[1:261],xlab='News Values',ylab='Weekly Changes in Stock
...
d_3=data.frame(a[1:261],trend$ASELSAN[5:265])
mod3=lm(a[1:261]~trend$ASELSAN[5:265], data=d_3)
summary(mod3)
plot(trend$ASELSAN[5:265],a[1:261],xlab='News Values',ylab='Weekly Changes in Stock
Prices',main='Changes in Stock Prices VS News with Regression
     Line')
abline(a=summary(mod3)$coefficients[1],b=summary(mod3)$coefficients
       [2])
...
ts2 <- ts(trend$ASELSAN ,frequency = 52)
